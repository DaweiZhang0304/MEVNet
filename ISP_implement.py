...
class ISP:
    def __init__(self, curve_path='/home/xteam_zdw/dn_g'):
    ...
    def noise_generate_srgb(self, img, configs='DND', is_mixup=False):
        # -------------------------- CAMERA SETTING --------------------------
    ...
    # def RAW2sRGB(self, img_mosaic, configs='DND', is_mixup=False):
    #     # -------------------------- CAMERA SETTING --------------------------
    #     cameras = self.cameras[configs]
    #     camera = cameras[random.randint(0, len(cameras)-1)]
...
def raw2stack_RGGB(var):
    h, w = var.shape
    res = np.zeros((4, h // 2, w // 2)).astype(np.float32)
    res[0] = var[0::2, 0::2]
    res[1] = var[0::2, 1::2]
    res[2] = var[1::2, 0::2]
    res[3] = var[1::2, 1::2]
...
# dataset = loadmat('/home/xteam_zdw/dataset/benchmarks/ValidationNoisyBlocksRaw.mat')
# img = dataset['ValidationNoisyBlocksRaw'][0,3,:,:]
# print(img.shape)
# # img = np.array(img['Inoisy'])
# isp = ISP()

# img = raw2stack_RGGB(img)


# # plt.imshow(isp.RAW2sRGB(img))
# plt.imshow(img[0], 'gray')
# plt.savefig('ss.png')
# plt.show()
# # # img = Image.open(f'results_sss8/Kodak24/bfnetr15_ab4b_5t20_nf/Gaussian/20/kodim04.png')
# # # img = np.array(img)/255
# # # 

# # # isp.noise_generate_srgb(img)
